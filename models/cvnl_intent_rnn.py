...
__all__ = ['IntentClassifierRNN', 'EmbeddingPackable']
...
clinc150 = requests.get("https://raw.githubusercontent.com/clinc/oos-eval/refs/heads/master/data/data_full.json")
clinc150Data = {}
if clinc150.status_code == 200:
  data = clinc150.json()
  print("Successfully fetched Clinc150 full data")
  #process the data
  for sentence, intent in (data["train"] + data["test"] + data["val"]):
    if intent in clinc150Data:
      clinc150Data[intent].append(sentence)
    else:
      clinc150Data[intent] = [sentence]


else:
  print("failed to fetch SNIPS Clinc150 data")
...
'''
def combineDataDicts(dataDicts, similarIntents = None):
  #merge dictionaries
  out = {}
  ...
  if similarIntents is None:
    return out

  #merge similar intents together
  for a, b in similarIntents:
    #just a simple check because im bound to make typos somewhere
    if not (a in out and b in out):
      print(f"intent {a} or intent {b} is not present")
      continue
...
similarIntents = [("restaurant_reservation", "BookRestaurant"), ("share_location", "ShareCurrentLocation"), ("greeting", "Greeting"), ("greeting", "CourtesyGreeting"),
 ("goodbye", "GoodBye"), ("goodbye", "CourtesyGoodBye"), ("Shutup", "cancel"), ("TimeQuery", "time"), ("directions", "GetDirections"), ("weather", "GetWeather"), ("traffic", "GetTrafficInformation"),
("CurrentHumanQuery", "user_name")]
finalData = combineDataDicts((snipsBuiltInData, chatbotData, clinc150Data), similarIntents)
...
#test pad_and_pack function
for batch in train_loader:
    break

## We first need to define these classes and import these modules to build our RNN
...
# Now, we want to include validation, hence 70% training. 15% test, 15% validation
...
trainingData, valData, testingData = torch.utils.data.random_split(dataset, [train_size, val_size, test_size])
...
train_loader = DataLoader(trainingData, batch_size=B, shuffle=True, collate_fn=pad_and_pack)
test_loader = DataLoader(testingData, batch_size=B, shuffle=False, collate_fn=pad_and_pack)
val_loader = DataLoader(valData, batch_size=B, shuffle=False, collate_fn=pad_and_pack)
...
num_classes = len(dataset.label_to_idx)  # how many distinct intents
vocab_size = len(dataset.vocab)         # from dataset's vocab

rnn_model = nn.Sequential(
    # Embedding layer wrapped in our custom EmbeddingPackable
    EmbeddingPackable(nn.Embedding(vocab_size, D)),   # (B, T) -> (B, T, D) with pack/unpack
    # Bidirectional GRU
    nn.GRU(
        input_size=D,
    ...
    # Use LastTimeStep utility to extract final hidden state
    LastTimeStep(rnn_layers=num_layers, bidirectional=bidirectional),
    nn.LayerNorm(hidden_nodes * 2),  # Critical normalization
    nn.Dropout(p=0.7),
